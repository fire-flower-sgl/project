...
@RestController
@CrossOrigin
@RequestMapping(value = "/MobileApi/UserController")
...
//	@Transactional(rollbackOn = Exception.class)
	public RespObject<?> loginIn(@RequestBody Map<String, String> map, HttpServletRequest request,HttpServletResponse response) {
		...
		String userName = "";
		String password = "";
		if (map == null || map.get("userName").isEmpty() || map.get("password").isEmpty()) {
...
//				+ "\n=========================");
	...
	public RespObject<?> updatePwd(@RequestBody JSONObject object) {
		...
		int flag = 0;
		// 取到数据用户
		User user = userService.selectSysPower(object.getString("id"));
		// 判断密码是否相等
		...
		if (user.getPassword().equals(reqPwd) && reqPwd.equals(newPwd)) {
			...
			if (user.getPassword().equals(reqPwd)) {
				...
				// 调用更新方法
				flag = userService.updateSysPower(user);
	...
	public boolean emailDr(String email, HttpServletRequest req) {
		...
		EmailParameter emailParameter = new EmailParameter(emailAddress, e.getSubject(), content, e.getName(),
				e.getPassword());
		boolean flag = emailService.sendSimpleMessage(emailParameter);//EmailUtils.sendSimpleMessage(emailParameter);
		if (!flag) {
			success = false;
		} else {
			// 发送成功后，将验证码存入
